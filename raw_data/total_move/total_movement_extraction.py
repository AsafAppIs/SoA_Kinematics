...
def all_subjects_data(list_mode=True):
    subjects_lst = []
    for experiment, participant in cfg.all_participants:
        ...
        movement, bad_trials = subject_total_movement(subject_kinematics)
        # add to list
...
def subject_total_movement(subject_kinematics):
    # define 2d array to conatin the total movement data from all trials
    # array dimentions (num of trials, num of points)
    movement = all_trials_total_movement(subject_kinematics)
    
    # filter anomalies
    subject_kinematics, num_exaggerated = filter_exaggerated_movements(subject_kinematics, movement)
    
    
    # filter extremes
    movement = all_trials_total_movement(subject_kinematics)
    subject_kinematics, num_of_big, num_of_small = filter_extreme_movements(subject_kinematics, movement)
...
# array dimentions (num of trials, num of points)
def all_trials_total_movement(subject_kinematics):
    # define 2d array to conatin the total movement data from all trials
    # array dimentions (num of trials, num of points)
    ...
    for i, trial in enumerate(subject_kinematics):
        movement[i] = trial_total_movement(trial[1])
...
def trial_total_movement(trial_df):
